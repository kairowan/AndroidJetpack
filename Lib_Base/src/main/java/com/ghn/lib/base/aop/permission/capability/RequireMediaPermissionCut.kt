 ...
 * 描述: RequireMediaPermissionCut 切面实现，负责统一拦截并编排相关横切逻辑。
 */

class RequireMediaPermissionCut : BasePointCutSuspend<RequireMediaPermission> {

    override fun invoke(joinPoint: ProceedJoinPoint, anno: RequireMediaPermission): Any? {
        return PermissionAopSupport.handle(joinPoint, anno.toConfig(), MEDIA_PERMISSIONS)
    }

    override suspend fun invokeSuspend(
        joinPoint: ProceedJoinPointSuspend,
        anno: RequireMediaPermission
    ) {
        PermissionAopSupport.handleSuspend(joinPoint, anno.toConfig(), MEDIA_PERMISSIONS)
    }
}
...
private fun RequireMediaPermission.toConfig(): PermissionRequestConfig {
