package com.kt.network.download.core

import android.content.Context
import android.util.Log
import com.kt.network.download.exceptions.HttpException
import com.kt.network.download.exceptions.RetryException
import com.kt.network.download.listener.DownloadListener
import com.kt.network.download.model.DownloadConfig
import com.kt.network.download.utils.FileUtils
import com.kt.network.download.utils.FileUtils.createTempFileForDownload
import com.kt.network.download.utils.FileUtils.getOriginalFilename
import com.kt.network.download.utils.PathUtils
import com.kt.network.download.utils.PathUtils.HttpClient
import com.kt.network.download.utils.PathUtils.ValidationUtils
...
class FileDownloader private constructor(
    ...
    companion object {
        fun create(
            context: Context,
            config: DownloadConfig = DownloadConfig.global
        ): FileDownloader {
            return FileDownloader(context, config)
        }
    }
    ...
    suspend fun execute(
        ...
        mutex.withLock {
            ...
            val basePath = config.allowedBasePath ?: PathUtils.getDefaultDownloadDir(context)
            ValidationUtils.validatePath(context, savePath, basePath.toString())
            // 生成唯一 ID
            ...
            val initialResponse = try {
                ...
                listener.onError(
                    downloadId,
                    IOException("Failed to get initial response: ${e.message}")
                )
                throw e
            }
            val originalFilename = getOriginalFilename(initialResponse)
            var (tempFile, targetFile) = createTempFileForDownload(context, originalFilename)
            var retryCount = 0
            while (retryCount <= config.maxRetries) {
                try {
                    ...
                    if (shouldRetry) {
                        retryCount++
                        tempFile.delete()
                        val (newTempFile, newTargetFile) = createTempFileForDownload(
                            context,
                            originalFilename
                        )
